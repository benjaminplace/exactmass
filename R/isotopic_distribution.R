#' Generate isotopic distribution mass spectrum
#'
#' @param elementalformula character string of chemical formula
#' @param max.dist numeric maximum absolute distance from the lowest m/z value to include in mass spectrum
#' @param min.int numeric minimum relative intensity to include in mass spectrum (between 0 and 1)
#' @param charge character vector of the charge state of the mass spectrum ("positive", "negative" or "neutral")
#'
#' @returns table containing m/z values (mz) and intensity values (int) of a predicted isotopic pattern
#' @export
#'
#' @examples
#' isotopic_distribution("C8F12O2", charge = "negative")
isotopic_distribution <- function(elementalformula, max.dist = 3, min.int = 0.001, charge = "neutral") {
  elementlist <- extract_elements(elementalformula)
